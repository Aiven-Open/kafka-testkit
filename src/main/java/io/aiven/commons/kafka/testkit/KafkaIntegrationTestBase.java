...
public class KafkaIntegrationTestBase {
  ...
  /**
   * The Test info provided before each test. Tests may access this info wihout capturing it
   * themselves.
   */
   ...
   * @param partition the partition for the record.
   * @param key the key for the recrod.
   * @param value the value for the record.
   * @return a {@code ProducerRecord<byte[], byte[]>}
   */
  protected static ProducerRecord<byte[], byte[]> recordOf(
      final String topic, final int partition, final String key, final String value) {
   ...
   */
  protected final String getConnectorName(final Class<? extends Connector> connectorClass) {
    String result = CONNECTOR_NAME_THREAD_LOCAL.get();
    if (result == null) {
      result =
          new CasedString(CasedString.StringCase.CAMEL, connectorClass.getSimpleName())
   ...
   */
  public final String getTopic(final String... args) {
  ...
  /**
   * Tears down any existing KafkaManager. if the KafkaManager has not be created no action is
   * taken.
   */
  protected final void tearDownKafka() {
   ...
   * @param connectorClass the connector class to use.
   */
  protected final void deleteConnector(final Class<? extends Connector> connectorClass) {
    final KafkaManager kafkaManager = KAFKA_MANAGER_THREAD_LOCAL.get();
    if (kafkaManager != null) {
   ...
   */
  protected final KafkaManager getKafkaManager() {
    final KafkaManager kafkaManager = KAFKA_MANAGER_THREAD_LOCAL.get();
    if (kafkaManager == null) {
   ...
   */
  @Deprecated
  protected final <K> void waitForStorage(
      final Duration timeout,
      final Supplier<Collection<K>> storageList,
